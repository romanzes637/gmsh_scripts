...
class TestTransform(unittest.TestCase):
    ...
    def test_1(self):
        ...
        p_car = Point(cs_car, [0, 0, 1])
        p_cyl = Point(cs_syl, [0, np.deg2rad(45), 1])
        ps_sph = Point(cs_sph, [1, np.deg2rad(45), np.deg2rad(60)])
        p_tor = Point(cs_tor, [1, np.deg2rad(30), np.deg2rad(45), 5])
        p_tok = Point(cs_tok, [1, np.deg2rad(30), np.deg2rad(45), 5, 1.1, 1.2])
        p_blo = Point(cs_blo, [0.5, -0.5, 0])
