...
def sort_object_only_shared_no_tool(factory, object_volumes, tool_volumes, shared_volumes):
    # FIXME workaround OCC bug: if number of objects and tools = 1 and there is no shared volumes,
    # OCC doesn't renumber volumes idxs, it follows to removing existing tool volume
    if len(object_volumes) == 1 and len(tool_volumes) == 1 and len(shared_volumes) == 0:
        pass
    else:
...
def primitive_by_primitive(factory, primitive_object, primitive_tool, remove_object=True, remove_tool=True,
                           sort_function=sort_object_no_shared_tool_shared, pre_boolean=True):
    if pre_boolean:
...
def primitive_by_primitive_return_shared(factory, primitive_object, primitive_tool, remove_object=True,
                                         remove_tool=True, sort_function=sort_object_no_shared_tool_shared,
                                         pre_boolean=True):
    if pre_boolean:
        result = primitive_pre_boolean(primitive_object, primitive_tool)
    else:
    ...
    for i, primitive_object in enumerate(complex_object.primitives):
        for j, primitive_tool in enumerate(complex_tool.primitives):
            ...
            primitive_by_primitive(factory, primitive_object, primitive_tool,
                                   remove_object, remove_tool, sort_function, pre_boolean)


def complex_by_complex_return_shared(factory, complex_object, complex_tool, remove_object=True, remove_tool=True,
                                     sort_function=sort_object_no_shared_tool_shared, pre_boolean=True):
    shared_volumes = set()
    for i, primitive_object in enumerate(complex_object.primitives):
        for j, primitive_tool in enumerate(complex_tool.primitives):
            print("{}/{} {} by {}/{} {}".format(
                i + 1, len(complex_object.primitives), primitive_object.physical_name,
                j + 1, len(complex_tool.primitives), primitive_tool.physical_name))
            shared_volumes.union(primitive_by_primitive_return_shared(factory, primitive_object, primitive_tool,
...
def complex_self_return_shared(factory, complex_object, pre_boolean=True):
    ...
    for i, c in enumerate(combinations):
        ...
        shared_volumes.union(primitive_by_primitive_return_shared(factory, complex_object.primitives[c[0]],
...
def primitive_by_complex(factory, primitive_object, complex_tool, remove_object=True, remove_tool=True,
    ...
    for i, primitive_tool in enumerate(complex_tool.primitives):
        ...
        primitive_by_primitive(factory, primitive_object, primitive_tool, remove_object,
                               remove_tool, sort_function, pre_boolean)


def primitive_by_complex_return_shared(factory, primitive_object, complex_tool, remove_object=True, remove_tool=True,
                                       sort_function=sort_object_no_shared_tool_shared, pre_boolean=True):
    shared_volumes = set()
    for i, primitive_tool in enumerate(complex_tool.primitives):
        print("{} by {}/{} {}".format(
        ...
        shared_volumes.union(primitive_by_primitive_return_shared(
            factory, primitive_object, primitive_tool, remove_object, remove_tool, sort_function, pre_boolean))
    return shared_volumes
...
def complex_by_primitive_return_shared(factory, complex_object, primitive_tool, remove_object=True, remove_tool=True,
                                       sort_function=sort_object_no_shared_tool_shared, pre_boolean=True):
    shared_volumes = set()
    for i, primitive_object in enumerate(complex_object.primitives):
        print("{}/{} {} by {}".format(
