            ...
            for tunnel in range(n_tunnels):
                ...
                for trench in range(n_trenches):
                    ...
                    for borehole in range(n_boreholes):
                        borehole_x = x - dx_borehole - borehole * dx_borehole
                        ...
                        coordinate = [level, tunnel, trench, 0, borehole]  # 0 - left side
                    ...
                    for borehole in range(n_boreholes):
                        ...
                        self.borehole_input['arguments']['transform_data'] = new_transform
                        self.boreholes.append(ComplexFactory.new(self.borehole_input))
                        coordinate = [level, tunnel, trench, 1, borehole]  # 1 - right side
                        self.boreholes_coordinates.append(coordinate)
...
if __name__ == '__main__':
    ...
    print('Initialize')
    nkm = TrenchNKM(**input_nkm['arguments'])
    nkm.synchronize()
    if not is_test:
        if factory_str == 'occ':
            nkm.evaluate()
