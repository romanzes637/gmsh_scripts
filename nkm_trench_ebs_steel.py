            ...
            for tunnel in range(n_tunnels):
                ...
                for trench in range(n_trenches):
                    ...
                    lengths = [
                        ...
                        (26.210, 1.510, 4.110, ebs_ls[1]),
                        (27.500, 2.800, 5.400, ebs_ls[2]),
                        (27.510, 2.810, 5.410, ebs_ls[3])]
                    physical_names = [
                        'Concrete',
                        'SteelInt',
                        'Bentonite',
...
if __name__ == '__main__':
    ...
    print('Gmsh options')
    if args.gmsh_settings is not None:
        print('Gmsh settings')
        with open(args.gmsh_settings) as f:
            gmsh_settings = json.load(f)
        pprint(gmsh_settings)
        factory_str = gmsh_settings['arguments']['factory']
        print(gmsh_settings['arguments']['options'])
        options = gmsh_settings['arguments']['options']
        ...
        # 1: Delaunay,
        # 4: Frontal, 5: Frontal Delaunay, 6: Frontal Hex,
        # 7: MMG3D, 9: R-tree, 10: HXT
        gmsh.option.setNumber('Mesh.Algorithm3D', 1)
        print('Mesh.Algorithm3D: {}'.format(
    ...
    print('Initialize')
    nkm = TrenchEbsSteelNkm(**input_nkm['arguments'])
    # nkm.synchronize()
    if not is_test:
        if factory_str == 'occ':
            nkm.evaluate()
